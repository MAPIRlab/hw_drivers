...
namespace SickToolbox {
  ...
  protected:
    ...
    virtual void _sendMessageAndGetReply( 
            const SICK_MSG_CLASS &send_message,
  ...
  void SickLIDAR< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_sendMessage( const SICK_MSG_CLASS &sick_message, const unsigned int byte_interval ) const 
  {
    ...
    if (byte_interval == 0) {
      ...
      if ((unsigned int)write(_sick_fd,message_buffer,message_length) != message_length) {      
	      throw SickIOException("SickLIDAR::_sendMessage: write() failed with byte_interval=0!");
      }
    }
    else 
    {
      /* Write the message to the unit one byte at a time */
      std::cout << "Writting to serial port a msg of legth " << message_length << std::endl;
      int valid_file = fcntl(_sick_fd, F_GETFD) != -1 || errno != EBADF;
      std::cout << "Serial port verification = " << valid_file << std::endl;

      for (unsigned int i = 0; i < message_length; i++) 
      {
        /* Write a single byte to the stream */
        std::cout << "Writting to serial port the value ["<< i << "] = '";
        std::cout << std::hex << &message_buffer[i] << "' " << std::endl;
      ...
      std::cout << "Done sending data over serial port" << std::endl;
    }
  }
   ...
   */
  template< class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
  void SickLIDAR< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_recvMessage( 
              SICK_MSG_CLASS &sick_message,
              const uint8_t * const byte_sequence,
              const unsigned int byte_sequence_length,
              const unsigned int timeout_value ) const {
    ...
    for(;;) 
    {
      ...
      /* Check whether the allowed time has expired */
      gettimeofday(&end_time,NULL);
      double elapsed_time = _computeElapsedTime(beg_time,end_time);
      if (_computeElapsedTime(beg_time,end_time) > timeout_value) {
        std::cout << "Timeout Waiting replay from SICK: " << elapsed_time << std::endl;
  ...
  void SickLIDAR< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_sendMessageAndGetReply( const SICK_MSG_CLASS &send_message,
    ...
    for(unsigned int i = 0; i < num_tries; i++) {
      try {
        ...
        /* Wait for the reply! */
        std::cout << "Waiting reply from SICK" << std::endl;
