...
inline SerialInterface::SerialInterface(const std::string& devicename, bool hardware_flow_control) : devicename(devicename),
...
inline bool SerialInterface::open()
{
    if (fd != -1) // is it alreay open?
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "[Giraff_ros_driver] %s", lastError.c_str());
        return false;
    }
    ...
    if (success)
    {
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "[Giraff_ros_driver] Successfully opened AVR on %s", devicename.c_str());
    ...
    if (!success)
    {
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "[Giraff_ros_driver] Error opening AVR on %s", devicename.c_str());
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "%s", strerror(errno));
...
inline GiraffPC::GiraffPC(const std::string& devicename) : SerialInterface(devicename, false),
initiated(false)
{
}
