...
public:
    ...
    *-------------------------------------------------------------------*/
    ...
    void getGiraffBatteryData(sensor_msgs::msg::BatteryState& bat);
...
private:
    ...
    rclcpp::Time t_last_velocity_command;
...
inline GiraffManager::GiraffManager(const std::string& giraffAVR_device, int32_t controller_mode, float maxLinVel, float maxAngVel, float acc_lin, float acc_ang, float vgr, std::string battery_technology, float battery_design_capacity, std::string battery_serial_number) : giraffAVR(giraffAVR_device),
...
batt_design_capacity(battery_design_capacity)
{
    ...
    t_last_velocity_command = rclcpp::Clock().now();
...
{
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "[giraff_ros_driver] Connecting to Giraff AVR");
    if (!giraffAVR.open())
    {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "[giraff_ros_driver] ERROR- Cannot open AVR port....Exiting");
    ...
    if (controllerMode == 0)
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "[giraff_ros_driver] Setting Initial State - Controller mode = 0");
    ...
    else
    {
        if (controllerMode != 2)
        {
            RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "[giraff_ros_driver] Controller mode unknow, setting default mode to 2");
            controllerMode = 2;
            state.mode = controllerMode;
        }

        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "[giraff_ros_driver] Setting Initial State - Controller mode = 2");
        if (!giraffAVR.setInt32("enc0", 0) ||
            !giraffAVR.setInt32("enc1", 0) ||
            !giraffAVR.setInt32("mode", controllerMode) ||
            !giraffAVR.setFloat("a", lin_acceleration) ||
...
{
    if (!giraffAVR.isOpen())
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "[Giraff_ros_driver] Cannot flush serial port (is it oppened?)");
    else
...
void GiraffManager::filterVelocity(float& linearVelocity, float& angularVelocity)
{
    ...
    rclcpp::Duration t_inc = t_now - t_last_velocity_command;
    float max_speed_inc_allowed, max_ang_speed_inc_allowed;
    // lin
    if (t_inc.seconds() < 1.0)
        max_speed_inc_allowed = lin_acceleration * t_inc.seconds();
    else
    ...
    // ang
    if (t_inc.seconds() < 1.0)
        max_ang_speed_inc_allowed = ang_acceleration * t_inc.seconds();
    else
...
inline GiraffState GiraffManager::setVelocity(float linearVelocity, float angularVelocity)
{
    ...
    bool use_twm = true; // giraffAVR.setTwm(vg, cdp, vgr, v, p, 0) ?
    float v, vg, p, vgr;
    int32_t r = 0;
    float cdp = 1.01; // Setting this just above the value of p, but a lower value does also seem to work.
    vgr = virtualGR;
    ...
    else if (controllerMode == 0)
    {
        ...
        if (linearVelocity > -0.001 && linearVelocity < 0.001 && angularVelocity > -0.001 && angularVelocity < 0.001)
        {
             ...
             * set mode 0
             * set a 1.0
             * set r 666 (Straight line - Not sure why they don't just set r to 1)
             * set p 0
             * set vgr 0
             */
        ...
        else
        {
            ...
            if (linearVelocity == 0.0)
            {
                /* From Camanio Tech Support
                 *  set mode 0
                 *  set a 0.35 // Acceleration.
                 *  set v 1.6 // Speed of rotation.
                 *  set r 0  // Rotate
                 *  set p 90 // Rotation in degrees.
                 */
                r = 0;                                                                                                            // Rotate
                v = angularVelocity > 0 ? (angularVelocity * DISTANCE_B_WHEELS / 2) : (-angularVelocity * DISTANCE_B_WHEELS / 2); // Speed of rotation
                p = angularVelocity > 0 ? -90.0 : 90.0;                                                                           // Rotation in degrees
                // vg = 0.0; //This is not relevant, any value should work

                // ROS_INFO("[Giraff_ros_driver] ROTATION - setting r=%i , v=%.3f , p=%.3f",r,v,p);
                if (!giraffAVR.setInt32("mode", 0) || !giraffAVR.setFloat("a", lin_acceleration) || !giraffAVR.setFloat("vgr", virtualGR) || !giraffAVR.setFloat("v", v) || !giraffAVR.setInt32("r", r) || !giraffAVR.setFloat("p", p))
                    throw GiraffManagerException(giraffAVR.getLastError());

                // updateState(mode, a, vgr, v, vg, p, r, lin_speed, ang_speed)
                updateState(0, lin_acceleration, virtualGR, v, state.vg, p, r, linearVelocity, angularVelocity);
            }

            // (B) Straight line (w=0)
            else if (angularVelocity == 0.0)
            {
                r = 50; // also r=666 is used by Camanio for straight line
                v = fabs(linearVelocity);
                p = linearVelocity > 0 ? 1 : -1;
                // vg = 0;  //This is not relevant, any value should work
            ...
            else
            {
                r = 100;
                v = fabs(linearVelocity);
                vg = (fabs(angularVelocity) * DISTANCE_B_WHEELS) / (2 * v - fabs(angularVelocity) * DISTANCE_B_WHEELS);

                // set signs according to V and W
                if (linearVelocity > 0 && angularVelocity > 0) // front + turn_left
                {
                    vg = -vg;
                    vgr = -vgr;
                    p = 1;
    ...
    {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "[Giraff Driver] Unknown Operation Mode!");
    }
...
void GiraffManager::getGiraffBatteryData(sensor_msgs::msg::BatteryState& bat)
{
    ...
    while (!msg_complete)
    {
        ...
        else if (!strcmp(parameter.c_str(), "W:F")) // parameter is battery watt/s
        {
            if (parameter_float == 0.0)
                bat.power_supply_status = sensor_msgs::msg::BatteryState::POWER_SUPPLY_STATUS_FULL; // fully_charged = 4
            else if (ch < 2)
                bat.power_supply_status = sensor_msgs::msg::BatteryState::POWER_SUPPLY_STATUS_CHARGING; // Charging = 1
            else
                bat.power_supply_status = sensor_msgs::msg::BatteryState::POWER_SUPPLY_STATUS_NOT_CHARGING; // Not Charging = 3
            has_ch = true;
        }
    ...
    bat.header.stamp = rclcpp::Clock().now();
    ...
    bat.percentage = NAN; // Charge percentage on 0 to 1 range  (If unmeasured NaN)
}
