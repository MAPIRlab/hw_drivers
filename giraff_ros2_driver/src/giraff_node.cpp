...
int main(int argc, char** argv)
{
    try
    {
        ...
        while (rclcpp::ok())
        {
            ...
            if (verbose) RCLCPP_DEBUG(rclcpp::get_logger("rclcpp"),"[Giraff_ros_driver] GiraffOdom [%f(m) %f(m) %f(rad)] [%f(m/s) %f(rad/s)]", pos_x,pos_y,yaw,lin_vel,ang_vel);
            ...
            {
                if (verbose) RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"[Giraff_ros_driver] TimeOut reached --> Stopping Robot");
